...
with selectGraph:
    ...
    xx=list(df)
    st.header("Input your filters:")
    dimension = st.selectbox("1D or 2D Graph:", ("1D",'2D'))
    gtype = st.selectbox("Select the Graph:", ("Beeswarm(1D)", "Scatter Plot (2D)", "Bar Graph (2D)"))
    xvalue = st.selectbox("Select the X Value: Ignore if 1D Graph", (list(df)))
    ...
    if createGraph:
        gnu=[]
        global graphobj
        graphobj=(dimension,gtype,xvalue,yvalue) 
        gnu.append(graphobj)
        st.text(gnu)
        # testing func
        # update with groups
